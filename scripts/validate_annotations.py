...
def check_suspicious_person_entities(entities: list[dict], text: str) -> list[dict]:
    ...
    job_title_keywords = [
        ...
        "Consultant",
        "Professeur",
        "Docteur",  # When standalone without a name
    ]
    ...
    for entity in entities:
        ...
        for keyword in job_title_keywords:
            if keyword in entity_text and not any(
                word[0].isupper() and word not in job_title_keywords
                for word in entity_text.split()[1:]
            ):
...
def check_suspicious_org_entities(entities: list[dict], text: str) -> list[dict]:
    ...
    for entity in entities:
        ...
        if any(
            ...
            for phrase in ["Je ", "Nous ", "Il ", "Elle ", "Mais "]
        ):
            suspicious.append(entity)
...
def main():
    ...
    for annotation_path in sorted(annotations_dir.glob("*.json")):
        ...
        if issues_found > 0:
            ...
            print(
                f"   - {result['suspicious_person_entities']} suspicious PERSON entities"
            )
