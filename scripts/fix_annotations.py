...
def is_false_positive_person(entity: Dict, text: str) -> bool:
    ...
    if any(
        entity_text.startswith(prefix)
        for prefix in ["Mon ", "Ma ", "Le ", "La ", "Les ", "Chez ", "En "]
    ):
        return True
    ...
    for keyword in org_keywords:
        if keyword in entity_text:
            ...
            if not re.match(
                r"^(?:Dr\.|Pr\.|M\.|Mme|Me|Professeur|Docteur)\s+[A-ZÀ-Ö]", entity_text
            ):
                ...
                proper_names = [
                    w
                    for w in words
                    if w and w[0].isupper() and w not in org_keywords and len(w) > 2
                ]
    ...
    if any(
        fragment in entity_text
        for fragment in ["égion", "ée également", "éveloppeuse,", "Mais avec"]
    ):
        return True
...
def is_false_positive_org(entity: Dict, text: str) -> bool:
    ...
    if any(
        entity_text.startswith(phrase)
        for phrase in ["Je ", "Nous ", "Il ", "Elle ", "Mais ", "Ensuite", "Avec "]
    ):
        return True

    # Contains verbs or common sentence structure
    if any(
        word in entity_text.lower()
        for word in ["suis", "avec", "pour", "pendant", "avant", "après"]
    ):
        return True
...
def remove_overlaps(entities: List[Dict]) -> List[Dict]:
    ...
    sorted_entities = sorted(
        entities, key=lambda x: (x["start_pos"], x["end_pos"] - x["start_pos"])
    )
    ...
    for entity in sorted_entities:
        ...
        for kept_entity in cleaned:
            # Check for overlap
            if not (
                entity["end_pos"] <= kept_entity["start_pos"]
                or entity["start_pos"] >= kept_entity["end_pos"]
            ):
                overlaps = True
                ...
                if (
                    entity_len < kept_len
                    and entity["entity_type"] == kept_entity["entity_type"]
                ):
...
def process_annotation_file(annotation_path: Path, corpus_dir: Path) -> Dict:
    ...
    if not doc_path.exists():
        return {"status": "error", "message": f"Source document not found: {doc_path}"}
...
def main():
    ...
    for annotation_path in sorted(annotations_dir.glob("*.json")):
        ...
        if result["status"] == "error":
            ...
            if result["removed"] > 0:
                print(
                    f"[FIXED] {annotation_path.name}: {result['original_count']} -> {result['new_count']} (-{result['removed']})"
                )
            else:
                print(
                    f"[OK] {annotation_path.name}: {result['new_count']} entities (no changes)"
                )
