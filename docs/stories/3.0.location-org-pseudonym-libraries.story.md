...
### Expand Organization Library (Priority: Medium)

**Issue Identified:** 2026-02-02 during batch processing of 15-document test corpus

The current neutral theme organization library has only **35 organizations** (20 companies + 10 agencies + 5 institutions). This is insufficient for batch processing larger document corpora - the library exhausts quickly and falls back to generic naming (Org-001, Org-002, etc.).

**Recommendation:**
- Expand neutral theme organizations to **150-200 entries**:
...
- Consider similar expansion for Star Wars and LOTR themes
- Source suggestions:
  - French business registry patterns (generic company names)
  - French government agency naming conventions
  - Fictional organization name generators

**Fallback Behavior:** The system now gracefully falls back to `Org-001`, `Org-002`, etc. when exhausted (fixed 2026-02-02), but themed pseudonyms are preferred for consistency.

**Related:** Location library (80 entries) may also benefit from expansion for very large corpora.
