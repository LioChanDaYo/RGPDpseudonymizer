...
**Required Stories:**
- ✅ Story 1.5: Walking Skeleton (Basic Process Command) - COMPLETE
- ✅ Story 1.6: NLP Integration with Basic Entity Detection - COMPLETE (verified 2026-01-20)
- ✅ Story 0.4: Validation UI/UX Design - COMPLETE

**Required Model Updates:**
- ✅ DetectedEntity.is_ambiguous field added (2026-01-20) for AC4 Step 3
...
- [ ] **Task 3: Implement Rich UI Components for Entity Display** (AC: 1, 2, 6)
  ...
  - [ ] Add color-blind safe palette (symbols + colors: ✓/✗/⚠ icons)
...
- [ ] **Task 5: Implement Entity Review Screen** (AC: 2, 3)
  ...
  - [ ] Implement navigation: Next (N/→), Previous (P/←), Jump (J+number)
...
- [ ] **Task 8: Implement Validation Workflow Orchestrator** (AC: 4, 5)
  ...
  - [ ] Step 2: Iterate through entity types (PERSON → ORG → LOCATION)
...
- [ ] **Task 9: Implement Final Confirmation Screen** (AC: 4)
  ...
  - [ ] Show example changes: "Marie → Leia Organa", "Rejected: TechCorp (false positive)"
...
- [ ] **Task 13: Implement Ambiguous Entity Flagging** (AC: 4)
  ...
  - [ ] Display ambiguous entities with warning icon ⚠ and yellow color coding
  - [ ] Show ambiguity reason (e.g., "Compound name", "Partial match", "Low confidence")
  - [ ] Prompt user to review ambiguous entities carefully: "⚠ Ambiguous entity. Confirm or modify?"
...
- [ ] **Task 15: Create Unit Tests for Validation Module** (AC: 8)
  ...
  - [ ] Test state transitions (PENDING → CONFIRMED → REJECTED)
...
- [ ] **Task 16: Create Integration Tests for Validation Workflow** (AC: 9)
  ...
  - [ ] Test full validation workflow: Summary → Review → Final Confirmation
...
- [ ] **Task 17: Implement Pagination for Large Entity Lists** (AC: 5, 7)
  ...
  - [ ] Navigation: Next Page (→), Previous Page (←), Jump to Page (J+page number)
...
- [ ] **Task 18: Conduct User Testing** (AC: 10)
  ...
  - [ ] Target success criteria: ≥4/5 rating on all dimensions, <2 min per document (20-30 entities)
...
- Validation workflow steps documented: Summary → Review → Flag Ambiguous → Final Confirmation
...
├── models.py           # ValidationSession, EntityReviewState data models
├── ui.py               # Rich UI components (EntityTable, ReviewScreen, SummaryScreen, HelpOverlay)
├── workflow.py         # ValidationWorkflow orchestrator
└── actions.py          # User action handlers (confirm, reject, modify, add, change_pseudonym)
```
...
- `ValidationWorkflow`: Orchestrates multi-step validation flow (summary → review → final confirmation)
...
- Iterate through entity types in priority order: PERSON → ORG → LOCATION
...
- Show warning icon ⚠ and yellow color coding
- Display ambiguity reason (e.g., "Compound name", "Low confidence")
- Prompt user to carefully review: "⚠ Ambiguous entity. Confirm or modify?"

**Step 4: Final Confirmation**
- Display summary of user decisions:
  - "Confirmed: 20 entities"
  - "Rejected: 2 false positives"
  - "Modified: 1 entity (Marie → Marie Dubois)"
  - "Added: 3 missed entities"
- Show example changes: "Marie Dubois → Leia Organa", "Rejected: TechCorp (false positive)"
- Prompt: "Ready to process document? [Y/n]"
...
| Action | Keyboard | Description | Implementation |
|--------|----------|-------------|----------------|
| **Next** | N or → | Move to next entity | Increment entity index, redraw screen |
| **Previous** | P or ← | Move to previous entity | Decrement entity index, redraw screen |
...
```
gdpr_pseudonymizer/
├── validation/                          (CREATE - new module)
│   ├── __init__.py                      (CREATE - module exports)
│   ├── models.py                        (CREATE - ValidationSession, EntityReviewState, UserDecision)
│   ├── ui.py                            (CREATE - Rich UI components)
│   ├── workflow.py                      (CREATE - ValidationWorkflow orchestrator)
│   └── actions.py                       (CREATE - User action handlers)
tests/
├── unit/
│   ├── test_validation_models.py        (CREATE - Data model tests)
│   ├── test_validation_ui.py            (CREATE - UI component tests)
│   ├── test_validation_workflow.py      (CREATE - Workflow tests)
│   └── test_validation_actions.py       (CREATE - Action handler tests)
├── integration/
│   └── test_validation_workflow_integration.py  (CREATE - Full validation workflow integration test)
docs/
├── validation-ui-spec.md                (EXISTING - Reference document)
└── README.md                            (MODIFY - Add validation workflow documentation)
```
...
def test_validation_session_get_validated_entities():
    ...
    session = ValidationSession(entities=entities, document_text="Marie Dubois travaille à Paris pour TechCorp.")
...
- Use symbols + colors: ✓ (green), ✗ (red), ⚠ (yellow)
...
**AC10 Requirements:** 2-3 target users test validation UI with real documents, measure validation time, collect feedback (≥4/5 rating target)
...
5. **Success criteria:**
   - ≥4/5 rating on all three dimensions (speed, clarity, ease of use)
...
5. **Validation Workflow Orchestrator** (Tasks 6-9, 11, 13)
   ...
   - Multi-step validation: Summary → Review by Type → Final Confirmation
   - Priority order: PERSON → ORG → LOCATION
...
✅ **PASS: Validation Time Target (<2 minutes)**
...
4. Entity-by-type grouping (PERSON → ORG → LOCATION) is logical
5. Context display is helpful for understanding entities

🔴 **Critical Issue Discovered - Duplicate Entity Validation:**
- **Problem:** Same entity text appears multiple times in document, requiring validation each time
- **Example:** "Marie Dubois" appears 10 times → user validates 10 times (extremely redundant)
...
**Gate: CONCERNS** → [docs/qa/gates/1.7-validation-ui-implementation.yml](../../qa/gates/1.7-validation-ui-implementation.yml)
...
- ✅ **Coding Standards:** PASS - Absolute imports, type hints, docstrings, no PII logging
- ✅ **Project Structure:** PASS - Follows unified structure (validation/ module, tests/unit/)
- ✅ **Testing Strategy:** PASS (with concerns) - 12 unit tests passing, integration tests deferred
- ✅ **All ACs Met:** PASS (with concerns on AC9, AC10) - See detailed AC assessment below
...
|----|--------|----------|-------|
| AC1 | ✅ PASS | [ui.py:10-18](../../gdpr_pseudonymizer/validation/ui.py#L10-L18) | Rich components implemented (Table, Confirm, Console, Panel) |
| AC2 | ✅ PASS | [ui.py:158-232](../../gdpr_pseudonymizer/validation/ui.py#L158-L232) | Entity presentation with grouping, context, confidence, pseudonym |
| AC3 | ✅ PASS | [ui.py:21-54](../../gdpr_pseudonymizer/validation/ui.py#L21-L54), [workflow.py:195-278](../../gdpr_pseudonymizer/validation/workflow.py#L195-L278) | All actions (confirm, reject, modify, add, change, batch) implemented |
| AC4 | ✅ PASS | [workflow.py:94-122](../../gdpr_pseudonymizer/validation/workflow.py#L94-L122) | 5-step workflow (Summary → Review → Ambiguous → Final → Process) |
| AC5 | ✅ PASS | [context_precomputer.py:62-81](../../gdpr_pseudonymizer/validation/context_precomputer.py#L62-L81) | Context precomputation + entity-by-type pagination |
| AC6 | ✅ PASS | [ui.py:295-339](../../gdpr_pseudonymizer/validation/ui.py#L295-L339) | Keyboard-only, color-blind safe (✓/✗/⚠), help overlay |
| AC7 | ✅ PASS | [workflow.py:62-76](../../gdpr_pseudonymizer/validation/workflow.py#L62-L76), [ui.py:341-370](../../gdpr_pseudonymizer/validation/ui.py#L341-L370) | Error handling (invalid keys, empty, 100+ warning) |
| AC8 | ✅ PASS | [test_validation_models.py](../../tests/unit/test_validation_models.py) | 12 unit tests, all passing (100% pass rate) |
| AC9 | ⚠️ CONCERNS | Task 16 deferred | Integration tests missing - only unit tests present |
| AC10 | ⚠️ CONCERNS | [Story completion notes](1.7.validation-ui-implementation.story.md#user-testing-results) | Self-testing ✅ (Speed 4/5, Clarity 5/5, Ease 5/5, <2 min met), external users deferred, **deduplication blocker discovered** |
| AC11 | ✅ PASS | [process.py:259-271](../../gdpr_pseudonymizer/cli/commands/process.py#L259-L271) | Validation mandatory, --validate flag removed |
...
#### 🔴 HIGH: Entity Deduplication Blocker (UX-001)
...
#### 🟡 MEDIUM: Integration Tests Missing (TEST-001)
...
- ✅ doc1 (10 entities): < 1 minute
- ✅ doc2 (20 entities): < 2 minutes
- ✅ doc3 (30 entities): < 2 minutes (target met!)
- ✅ doc4 (50 entities): < 2 minutes (very fast)
- ⚠️ doc5 (100 entities): Skipped due to duplicate validation fatigue
...
|-----|--------|------------|
| **Security** | ✅ PASS | No PII logged, no hardcoded secrets, input validation present |
| **Performance** | ✅ PASS | <2 min target met for 20-30 entities, context precomputation works well |
| **Reliability** | ✅ PASS | Comprehensive error handling, graceful fallbacks, robust state management |
| **Maintainability** | ✅ PASS | Excellent code organization, comprehensive docs, type hints, clean architecture |
| **Usability** | ⚠️ CONCERNS | UI/UX excellent, but duplicate validation creates friction for large docs |
| **Scalability** | ⚠️ CONCERNS | Works well 1-50 entities, degrades 50-100+, blocked by deduplication issue |
...
⚠️ **Changes Required - See Issue UX-001 (Deduplication)**
...
**Next Steps:**
1. ✅ Story 1.9 created for deduplication enhancement
2. ✅ Story 1.7 marked as "Done" with documented limitation
3. Production release includes both 1.7 + 1.9

**Story 1.7 Status:** DONE ✅
