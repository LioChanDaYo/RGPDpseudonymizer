...
- Implemented Windows-specific CI strategy: runs linting/type checks on all platforms, full tests on Linux/macOS, non-spaCy tests on Windows
...
- Created run-quality-checks.sh script for local CI/CD validation
...
- [.github/workflows/ci.yaml](../../.github/workflows/ci.yaml) - Configured Windows to skip spaCy tests, run linting on all platforms

**New Files Created:**
- [tests/conftest.py](../../tests/conftest.py) - Added pytest configuration with OMP_NUM_THREADS=1 for local test runs
- [run-quality-checks.sh](../../run-quality-checks.sh) - Local quality checks script matching CI/CD pipeline
