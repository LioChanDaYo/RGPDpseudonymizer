...
- [ ] **Task 5.2.1: Build gender lookup dictionary data file** (AC: 1)
  - [ ] Create `data/french_gender_lookup.json` with structure:
    ```json
    {
      "data_sources": [...],
    ...
    ```
  - [ ] Populate from existing pseudonym library data: extract all unique first names from `neutral.json` `first_names.male` (411 names) and `first_names.female` (372 names) sections. Note: `first_names.neutral` is currently an empty array -- no names to extract from it
  - [ ] Supplement with names from `french_names.json` first_names list, classifying them by gender using the pseudonym library as reference
  - [ ] Identify ambiguous/unisex names (e.g., Camille, Dominique, Claude, Sacha, Lou, Eden) -- place in `ambiguous` list
  - [ ] Document INSEE Open License 2.0 (Etalab) compatibility with MIT in `data_sources` field
  - [ ] Target: >=500 gendered names (>=250 male + >=250 female), matching or exceeding the coverage of common French first names
  - [ ] Atomic commit

- [ ] **Task 5.2.2: Bundle gender lookup data in package resources** (AC: 1)
  - [ ] Copy `data/french_gender_lookup.json` to `gdpr_pseudonymizer/resources/french_gender_lookup.json`
  - [ ] Add `FRENCH_GENDER_LOOKUP_PATH` constant to `gdpr_pseudonymizer/resources/__init__.py`
  - [ ] Follow existing pattern: `FRENCH_GENDER_LOOKUP_PATH = RESOURCES_DIR / "french_gender_lookup.json"`
  - [ ] Atomic commit

- [ ] **Task 5.2.3: Create `GenderDetector` module** (AC: 1, 4)
  - [ ] Create `gdpr_pseudonymizer/pseudonym/gender_detector.py`
  - [ ] Class `GenderDetector`:
    ...
    class GenderDetector:
        ...
        def detect_gender_from_full_name(self, full_name: str, entity_type: str) -> str | None:
            ...
            """
    ```
  - [ ] Case-insensitive matching (normalize with `.capitalize()`)
  - [ ] Compound name handling (AC4): split on `-`, use first component
    - "Jean-Pierre" -> lookup "Jean" -> male
    - "Marie-Claire" -> lookup "Marie" -> female
    - "Jean-Marie" -> lookup "Jean" -> male (first component wins)
  - [ ] Ambiguous names return `None` (fallback to combined list = no regression)
  - [ ] Unknown names return `None` (same fallback)
  - [ ] Load lazily on first call to `detect_gender()` or explicitly via `load()`
  - [ ] Use `from gdpr_pseudonymizer.resources import FRENCH_GENDER_LOOKUP_PATH` for data path
  - [ ] Atomic commit

### Phase 2 -- Pipeline Integration (AC: 2, 3)

- [ ] **Task 5.2.4: Integrate gender detection into `CompositionalPseudonymEngine`** (AC: 2, 3)
  - [ ] Add `GenderDetector` as an optional dependency to `CompositionalPseudonymEngine.__init__()`:
    ```python
    def __init__(
        self,
    ...
    ```
  - [ ] In `assign_compositional_pseudonym()`: if `gender` param is `None` AND `gender_detector` is available, auto-detect gender from `entity_text`
  - [ ] Pass detected gender through to `pseudonym_manager.assign_pseudonym(gender=detected_gender)` -- this satisfies **AC3** (female entity -> female pseudonym, male entity -> male pseudonym, unknown -> combined list)
  - [ ] **CRITICAL:** Do NOT change `assign_compositional_pseudonym()` signature -- it already accepts `gender: str | None`. The change is internal: auto-detect when `None` is passed.
  - [ ] Atomic commit

- [ ] **Task 5.2.5: Integrate gender detection into `DocumentProcessor`** (AC: 2)
  - [ ] In `_init_processing_context()`: create `GenderDetector()` instance and pass to `CompositionalPseudonymEngine`:
    ```python
    ...
    ```
  - [ ] **No changes needed** to `_build_pseudonym_assigner()`, `_assign_new_pseudonym()`, or `_resolve_pseudonyms()` -- they already pass `gender=None` which triggers auto-detection inside `CompositionalPseudonymEngine`
  - [ ] Atomic commit

- [ ] **Task 5.2.6: Store detected gender in Entity records** (AC: 2)
  - [ ] In `_assign_new_pseudonym()` method of `DocumentProcessor`:
  ...
  - [ ] For reused entities (existing in DB): gender remains as-is from the existing record
  - [ ] **Design Decision:** For entities processed before this story (gender=None in DB), do NOT backfill. Gender will be populated for all new entities going forward. Backfilling would require re-processing documents.
  - [ ] Atomic commit

### Phase 3 -- Unit Tests (AC: 5)

- [ ] **Task 5.2.7: Unit tests for `GenderDetector`** (AC: 5)
  - [ ] Create `tests/unit/pseudonym/test_gender_detector.py`
  - [ ] Tests:
  ...
  - [ ] Follow existing test patterns with `pytest` fixtures
  - [ ] Atomic commit

- [ ] **Task 5.2.8: Unit tests for gender-aware pseudonym assignment** (AC: 5)
  - [ ] Create `tests/unit/pseudonym/test_gender_aware_assignment.py`
  - [ ] Tests:
  ...
  - [ ] Use `LibraryBasedPseudonymManager` with `neutral` library loaded
  - [ ] Atomic commit

- [ ] **Task 5.2.9: Unit tests for gender storage in Entity** (AC: 5)
  - [ ] Extend or create `tests/unit/core/test_document_processor_gender.py`
  - [ ] Tests:
  ...
  - [ ] Use mock NLP detector and mock pseudonym manager (consistent with existing DocumentProcessor test patterns)
  - [ ] Atomic commit

### Phase 4 -- Integration Test (AC: 6)

- [ ] **Task 5.2.10: Integration test for gender-aware processing** (AC: 6)
  - [ ] Create `tests/integration/test_gender_aware_processing.py`
  ...
  - [ ] Use `CliRunner` or direct `DocumentProcessor` API
  - [ ] Skip if spaCy model not available (Windows CI compatibility)
  - [ ] Atomic commit

### Phase 5 -- Documentation (AC: 8)

- [ ] **Task 5.2.11: Update methodology documentation** (AC: 8)
  - [ ] Update `docs/CLI-REFERENCE.md` or relevant methodology page:
...
- [ ] **Task 5.2.12: Full regression suite** (AC: 7)
  ...
  - [ ] Verify test count >= 1133 baseline (Story 5.1 count)
  - [ ] Verify existing `process`, `batch`, `destroy-table` commands unaffected
  - [ ] Verify existing mappings with `gender=None` remain valid and queryable
  - [ ] Verify `--help` renders correctly
